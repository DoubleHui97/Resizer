...
def main(cfg: DictConfig):
    ...
    testloader = torch.utils.data.DataLoader(
        testset, batch_size=100, shuffle=True, num_workers=8)
    ...
    if net.resizer_model is not None:
        ...
        state_dict = torch.load(CKPT_ROOT + 'ckpt/cifar10/resnet50_resizer/checkpoint.pth')
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets) in enumerate(testloader):
                for i in range (0, 100):
                    ...
                    save_images(input0, CKPT_ROOT, sample_name=classes[target0] + '_cifar10')
                    save_images(mid_output0, CKPT_ROOT, sample_name=classes[target0] + '_cifar10_mid_output')
