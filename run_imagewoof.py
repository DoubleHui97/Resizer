...
def main(cfg: DictConfig):
    ...
    trainset = torchvision.datasets.ImageFolder(
        root=cfg.trainer.data_path+'train',transform=transform_train)
    trainloader = torch.utils.data.DataLoader(
        trainset, batch_size=cfg.data.batch_size, shuffle=True, num_workers=8)

    testset = torchvision.datasets.ImageFolder(
        root=cfg.trainer.data_path+'val', transform=transform_test)
    testloader = torch.utils.data.DataLoader(
        testset, batch_size=cfg.data.batch_size, shuffle=False, num_workers=8)
    ...
    if cfg.trainer.checkpoint_path != '':
        ...
        checkpoint_path = os.path.join(CKPT_ROOT + 'ckpt/imagewoof2/' + str(cfg.trainer.arch) + '_'
